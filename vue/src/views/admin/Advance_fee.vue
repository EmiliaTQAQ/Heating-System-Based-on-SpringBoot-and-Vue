<template>
  <div>
    <div style="margin-left: 550px;margin-bottom: 20px">
      ...
      <el-button type="danger" round style="margin: 10px" @click="transfer()">数据转移至缴费记录</el-button>
    </div>
    <div style="width: 100%">
...
export default {
  ...
  methods: {
    findBySearch() {
      ...
      request.get("/advance_fee/search", {
        params: this.params
      }).then(res => {
        if (res.code === '0') {
    ...
    del() {
      request.delete("/advance_fee").then(res => {
        if (res.code === '0') {
    ...
    transfer(){
      request.delete("/pay_the_fees").then(res => {
        if (res.code === '0') {
          request.post("/advance_fee/transfer").then(res => {
            if (res.code === '0') {
              request.delete("/advance_fee").then(res => {
                request.post("/room/gx",).then(res => {
                  //温度控制
                  request.post("/temperature/in",).then(res => {
                  })
                })

              })
              this.$message.success("转移成功成功")
            } else {
              this.$message.error(res.msg)
            }
          })
